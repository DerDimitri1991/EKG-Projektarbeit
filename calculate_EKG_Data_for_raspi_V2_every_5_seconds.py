...
class ECGDeviceHandler:
    def __init__(self, device_id):
        ...
        threading.Thread(target=self.periodic_processor, daemon=True).start()
    ...
    def periodic_processor(self):
        while True:
            ...
            if len(self.buffer_ecg) > 100 and len(self.recent_rates) >= 5 and not self.processing:
                with self.lock:
                    signal = self.buffer_ecg.copy()
                    rate = np.mean(self.recent_rates)
                threading.Thread(
